...
import com.example.claims.android.scan.runOcr
...
class ScanViewModel(private val repo: ClaimsRepository) : ViewModel() {
    ...
    /** A page was scanned: OCR it on-device, then deterministically parse into the form. */
    fun onScanned(context: Context, uri: Uri) {
        _state.update { it.copy(imageUri = uri, busy = true, error = null) }
        runOcr(
            context, uri,
            onText = { text ->
                _state.update { it.copy(form = ClaimParser.parse(text), hasOcr = true, busy = false) }
            },
            onError = { e ->
